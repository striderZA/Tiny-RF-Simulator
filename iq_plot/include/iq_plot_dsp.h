...
inline bool build_iq_spectrum(const std::vector<double> &frequencies,
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        out_spectrum[i] =
            std::complex<double>(magnitude * std::cos(phase_rad), magnitude * std::sin(phase_rad));
    }
    ...
    for (const auto &tone : tones) {
        ...
        if (best_idx >= 0 && best_idx < static_cast<int>(N)) {
            ...
            out_spectrum[best_idx] +=
                std::complex<double>(mag * std::cos(phase_rad), mag * std::sin(phase_rad));
        }
    }
