...
- `ComponentLibrary::loadFile()` / `scan()` continue discovery across malformed files: definitions with valid required fields (`type`, `part_number`, `parameters`) are retained, with malformed optional metadata logged and defaulted (`schema_version` → 1; non-string `manufacturer`/`description`/`notes` → empty); definitions with missing or wrong-typed required fields are logged and dropped; malformed `data_files` children are logged and skipped individually so valid siblings are kept; filesystem errors are logged without aborting later siblings, and no JSON or filesystem exception propagates out of the library layer
