...
- `ComponentLibrary::loadFile()` / `scan()` skip malformed definitions/files and continue discovery: invalid entries are logged and dropped, filesystem errors are logged, and no JSON or filesystem exception propagates out of the library layer to abort later siblings
