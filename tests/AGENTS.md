...
- **test_issue45_extension_trust.cpp** — standalone executable: issue #45 extension trust gating (`ExtensionTrustStore` persistence/fail-closed loading including canonical keys for hand-edited rows and refused incomplete approvals, project-local-external-tool-only gate, refuse/grant/deny/revoke, a failed write leaving no memory-only approval, and the `toolsMenuEntries()` menu payload) and first-root-wins `Shadowed` duplicate ids; app-level cases must redirect `m_extension_trust` to a temp store; kept out of the main `tests` binary for the MinGW registration ceiling
