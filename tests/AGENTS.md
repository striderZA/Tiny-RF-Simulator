...
- **test_issue116_collapsed_groups.cpp** — standalone executable: issue #116 graph-topology fixes (collapsed-group blocks render from the cached grid positions, links between two different collapsed groups are drawn through both boundary pins, and the editor's `canAddLink` policy rejects duplicate-input and cyclic links); kept out of the main `tests` binary for the MinGW registration ceiling
