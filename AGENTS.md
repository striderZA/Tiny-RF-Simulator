...
- The `package` job builds **and tests** the Linux/Windows artifacts attached to a GitHub release with `CMAKE_BUILD_TYPE=Release` (non-UI CTest, plus the MinGW TEST_CASE registration floor on Windows); it runs on every tag, so the shipped Release configuration is always exercised. Debug builds are validation-only and are never shipped.
