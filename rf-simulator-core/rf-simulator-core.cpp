...
void RfSimulatorCore::MainLoop(const std::function<void()>& onGui) {
	while (!glfwWindowShouldClose(p_impl->window)) {
		...
		ImGui::DockSpaceOverViewport(ImGui::GetID("MainDockSpace"), nullptr, ImGuiDockNodeFlags_PassthruCentralNode);
		...
		glClearColor(p_impl->clearColor.x * p_impl->clearColor.w, p_impl->clearColor.y * p_impl->clearColor.w, p_impl->clearColor.z * p_impl->clearColor.w, p_impl->clearColor.w);
